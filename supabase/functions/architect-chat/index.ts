...
3. Любые «настраиваемые тексты» (меню, прайс, расписание, контакты) клади в глобальные переменные {var.X} через set_variables. Владелец сможет менять их без передеплоя через `/set <key> <value>`.
...
D. Каждая видимая кнопка keyboard.inline куда-то ведёт через trigger.callback/add_menu_section, кроме магической кнопки open_miniapp — она открывает Mini App напрямую и НЕ требует trigger.callback.
