...
КОРЗИНА И ЗАКАЗЫ: если Mini App — это МАГАЗИН (пиццерия, доставка, маркетплейс) или БРОНИРОВАНИЕ — включай корзину через set_miniapp_cart({enabled: true}) И задавай priceNumeric у каждого добавляемого товара. Корзина без priceNumeric не работает (нечего считать). Для информационных Mini App (FAQ, VPN-сервер, профиль) корзина не нужна — не вызывай set_miniapp_cart.
