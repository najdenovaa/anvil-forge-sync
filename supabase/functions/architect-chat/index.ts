...
5. add_miniapp_item({ title, subtitle?, meta?, emoji?, badge?, badgeColor?, image? }) — ОТДЕЛЬНО для каждого товара/услуги. title — название, subtitle — короткое описание, meta — цена/длительность, emoji — короткий значок (используется если нет image), badge — необязательная метка («Хит», «Новинка»), badgeColor — hex-цвет фона бейджа («#22c55e» зелёный для «Новинка», «#ef4444» красный для «Скидка», «#f59e0b» янтарный для «Хит»), image — URL картинки товара (для layout=grid это главный визуал тайла).
  ...
  tools.push(
    ...
    },
    {
      ...
      function: {
        ...
        description:
          "Добавить ОДИН элемент в список items Mini App (товар, услуга, локация). title — обязателен, subtitle — описание, meta — цена / длительность / любое мета-поле, emoji — короткий эмодзи в левом углу карточки, badge — текст бейджа ('Хит', 'Новинка'), badgeColor — hex-цвет фона бейджа (например '#22c55e' зелёный для 'Новинка', '#ef4444' красный для 'Скидка'), image — URL квадратной картинки товара (для grid-layout — главный визуал тайла). Вызывай ОТДЕЛЬНО для каждого элемента.",
        parameters: {
          type: "object",
          properties: {
            title: { type: "string" },
