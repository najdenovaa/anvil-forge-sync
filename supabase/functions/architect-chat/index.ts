...
9. set_miniapp_profile_fields({ fields: [{ label, key?, value?, icon? }, ...] }) — явная привязка вкладки «Профиль» к user_var. Для имени/телефона НЕ пиши произвольные profileName/profilePhone/userVars в miniapp — они не рендерятся. Используй fields: [{label:"Имя", key:"user_name"}, {label:"Телефон", key:"user_phone"}] или канонические key "name"/"phone" если именно так сохранено в action.input/action.set_user_var.
