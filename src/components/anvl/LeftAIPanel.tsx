...
import {
  safeParseAnvlBlueprint,
  type AnvlBlueprint,
  type AnvlBlueprintNode,
...
const MODELS: ModelDef[] = [
  ...
  {
    id: "claude",
    labelKey: "ai.model.claude",
    short: "CL",
    routed: true,
    accent: "oklch(0.72_0.16_60)",
  },
  ...
  {
    id: "grok",
    labelKey: "ai.model.grok",
    short: "GROK",
    routed: true,
    accent: "oklch(0.72_0.18_30)",
  },
];
...
function describeToolStep(name: string, args: Record<string, any>): string {
  switch (name) {
    ...
    case "set_miniapp_cart":
      return args.enabled
        ? `Включил корзину (action=${args.sendAction ?? "order"})`
        : "Выключил корзину";
    default:
      return name;
  }
...
function stripTaggedBlocks(source: string) {
  return source
    ...
    .replace(/<blueprint>[\s\S]*?<\/blueprint>/gi, "")
...
function buildRunnableCodeFromTools(ops: ToolOp[], platform: string) {
  const nodes = new Map<
    string,
    { kind: string; title: string; preview: string; params: Record<string, string> }
  >();
...
function sanitizeBlueprintForMode(
  blueprint: AnvlBlueprint,
...
export function LeftAIPanel() {
  ...
  const [messages, setMessages] = useState<Msg[]>(() =>
    loadPersistedMessages(slug, t("ai.msg.intro")),
  );
  ...
  const runRound = async (
    ...
    const applyToolCall = (rawName: string, argsRaw: string) => {
      ...
      if (name === "reset_canvas" && liveStep < 1) liveStep = 1;
        ...
        (name === "set_preview" || name === "set_miniapp" || name === "set_param") &&
        liveStep < 3
      )
        liveStep = 3;
      try {
        ...
        else if (name === "set_miniapp_stats")
        ...
        else if (name === "set_miniapp_cart") setMiniAppCart(args as any);
        ...
        console.warn("tool apply failed", name, err);
      }
    };
    ...
    const flush = () => {
      if (
        !blueprintApplied &&
        blueprintRaw.includes('"nodes"') &&
        blueprintRaw.includes('"preview"')
      ) {
      ...
      setMessages((prev) => {
        ...
        const synthesised = liveSteps.length ? liveSteps.map((s) => "• " + s).join("\n") : "";
  ...
  const send = async (override?: string) => {
    ...
    try {
      ...
      while (!currentResult.usedTools && !currentResult.finalAnswer.trim() && retryAttempt < 2) {
        retryAttempt++;
        ...
        {phases.map((p, i) => {
          ...
          return (
            <li key={p.key} className="flex items-center gap-1.5">
              ...
              {i < phases.length - 1 && <span className="mx-0.5 h-px w-3 bg-hairline" />}
            </li>
          );
        })}
      </ol>
...
function ToolOpsFeed({ ops, live = false }: { ops: ToolOp[]; live?: boolean }) {
  ...
  const summarize = (op: ToolOp): string => {
    ...
    switch (op.name) {
      case "reset_canvas":
        return t("ai.tools.reset_canvas");
      case "add_node":
        return `${t("ai.tools.add_node")}: ${a.title ?? a.kind ?? a.id ?? ""}`;
      case "connect":
        return `${t("ai.tools.connect")} ${a.from ?? "?"} → ${a.to ?? "?"}`;
      case "set_param":
        return `${t("ai.tools.set_param")} ${a.id ?? ""}.${a.key ?? ""} = ${String(a.value ?? "").slice(0, 30)}`;
      case "set_preview":
  ...
  return (
    <div
      className={cn(
        "rounded-xl border border-hairline bg-surface/60 px-3 py-2",
        live && "border-foreground/20",
      )}
    >
      <button
        onClick={() => setOpen((v) => !v)}
